...
### A. Resend 메일 연결
- 도메인 인증 → 문의 알림 + 피드백 알림 발송

### B. /contact 또는 about에 문의 폼
- 운영자 이메일로 문의 수신

### C. 푸터 정비
- 이용약관, 개인정보처리방침, 문의 링크 정리

### D. 새 제품 등록 (운영 액션, 코드 작업 X)
- v2 필드 채워서 등록 (운영자 등록권 잔액: 현재 2개)

### E. 메이커 자기 제품 삭제 시 등록권 환불 정책 결정 + 구현
- 제품 삭제 시 등록권 환불 여부 → 형이 결정 후 구현
- 현재: 삭제해도 크레딧 잔액 변화 없음

### F. 등록 폼 SPA 사이트 AI 추출 개선
- 메타 태그 없는 SPA 사이트에서 더 많은 정보 추출
- 현재: ≤2개 필드 채워지면 경고 화면 노출

### G. prefers-reduced-motion 사이트 전체 점검
- HeroSlideshow 외 다른 모션(애니메이션, 트랜지션) 있는 컴포넌트 확인
- 첫 적용: HeroSlideshow (2026-04-27)

### H. 고아 파일 정리 (Storage, 우선순위 낮음)
...
- 서비스 정체성: "메이커 피드백이 본질, 등록 증명은 안전장치" (피드백 우선)
- 영문 브랜드명: **MyProduct** (CamelCase)
- 카테고리: 한국어 라벨 노출, value는 snake_case
...
- **슬라이드 콘텐츠**: 헤드라인 + 서브헤드 + 디테일 3개 불릿 구조 (피드백/안전/발견)
- **prefers-reduced-motion**: 사이트 전체 첫 적용 (components/home/HeroSlideshow.tsx)
- **좌측 카피 간략화**: 등록 즉시/동료 메이커 두 줄 제거 → 슬라이드로 이동
- **푸터 저작권 영어화**: "마이프로덕트. 모든 권리 보유." → "MyProduct. All rights reserved."
